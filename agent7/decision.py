...
6. When the goal involves making a file or document searchable for future queries, use
   index_document — it chunks and embeds the content into persistent vector memory.
   For one-shot reading of a file's raw contents, use read_file instead.
7. When memory hits include indexed fact chunks (kind=fact, source contains "index_document"),
   use search_knowledge to retrieve relevant chunks by semantic query rather than
   re-reading the original source files.
8. ANTI-LOOP RULE: If search_knowledge appears in RECENT HISTORY, synthesize an answer
   from those chunk results immediately — do NOT call search_knowledge again. The chunk
   content in history IS your source material. Read it carefully and answer from it.
"""
...
def next_step(
    ...
    for ev in history[-8:]:
        if ev.get("kind") == "action":
            # Use 1500 chars for RAG tools so full chunk content is visible to Decision
            trunc = 1500 if ev.get("tool") in ("search_knowledge", "index_document") else 300
            hist_lines.append(
                f"  iter{ev['iter']} {ev['tool']}"
                f"({str(ev.get('arguments', ''))[:60]}) "
                f"→ {ev.get('result_descriptor', '')[:trunc]}"
            )
